...
class Base():
	...
	def__init__(self, price, tansmission ='automatic', color='white'):
	self.price = price
...
coop.info()
print('The %s is gets %4. if miles per gallon' % (coop, coop.miles_per_gallon(coop.miles_range, coop.tank_capacity)))
print('The %s is gets %4. if miles per gallon' % (Base, Base.miles_per_gallon(Base.miles_range, Base.tank_capacity)))

class Sport(Base):
	''' Represent the sport model of a car based on base model'''
	engine_liters = 2.0
	miles_range = 400
	
sport = sport(color='red', transmission='manual', price=26300)
sport.info()
print('The %s is gets %4. if miles per gallon' % (sport, sport.miles_per_gallon(sport.miles_range, sport.tank_capacity)))
print('The %s is gets %4. if miles per gallon' % (Sport, Sport.miles_per_gallon(Sport.miles_range, Sport.tank_capacity)))
